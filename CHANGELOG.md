...
### Changed (Wiktionary wiring is now auditable + race-free)
- `annotator/google_apps_script.gs`: `/translate` response now includes a `context_used` block (`word_defs_count`, `had_modern_tamil_context`, `had_english_context`, `classical_tamil_chars`) so the frontend can confirm what context actually reached the prompt without checking the Sheet's `_telemetry` tab.
- `annotator/index.html`: `requestTranslation` now `await`s the Wiktionary prefetch promise before POSTing — fixes a race where clicking Suggest faster than the parallel Wiktionary lookups completed (~1.5s) sent an empty dictionary section even when entries existed. Also logs the outgoing request and the backend's confirmed `context_used` to the browser console so the operator can verify the wiring end-to-end in DevTools.

### Changed (default translation model: Llama 3.3 instead of Nemotron)
- `annotator/google_apps_script.gs`, `annotator/SETUP.md`, `docs/decisions/0007-translation-backend.md`: `TRANSLATION_MODEL` default changed from `nvidia/nemotron-3-nano-30b-a3b` to `meta/llama-3.3-70b-instruct`. Nemotron 3 Nano on NIM is a reasoning model and ignores `enable_thinking=false` in the configurations we tested — its raw chain-of-thought leaks into the response (we observed it producing "We need to parse… Let's try…" prose instead of the requested translation). Llama 3.3 70B is non-reasoning, has broad multilingual coverage, and produces clean direct answers under the same NIM API. Phase 5 alignment with Nemotron is preserved architecturally (NIM, same `/translate` proxy contract); the model identifier is swappable via Script Property, and the eval in ADR 0008 will inform the eventual production winner.
- `annotator/google_apps_script.gs`: system prompt strengthened to explicitly forbid analysis, parsing, line-by-line breakdowns, transliteration, commentary, and meta-talk preambles. Output format now states "Output ONLY the final translation. Nothing else." This is defense-in-depth in case any swapped-in model has reasoning-leakage tendencies.

### Changed (CI auto-redeploy temporarily disabled)
- `.github/workflows/deploy-backend.yml`: the `clasp version` and `clasp redeploy` steps are commented out. Empirically these reset the deployment's "Who has access" setting from "Anyone" to a non-Anyone default on every run, breaking the public `/exec` endpoint and creating a fix-push-reset loop where the operator can never get a successful CI run. Code pushes via `clasp push --force` still happen on every CI run; publishing to `/exec` is now a manual step (Apps Script editor → Manage deployments → edit → Version: New version → Deploy). The "Who has access" setting persists across UI-driven redeploys, so this only needs the one-time UI fix when a new version is needed live. Long-term fix is to replace these steps with direct `script.googleapis.com` API calls that explicitly set `entryPoints[0].webAppConfig.access = "ANYONE"` in the request body — that's a separate session of work. The post-deploy probe step remains in place to catch any access reversion.

### Fixed (Nemotron 3 Nano reasoning-mode handling)
- `annotator/google_apps_script.gs`: the Nemotron 3 Nano family is a *reasoning* model. The previous parameter `detailed_thinking: false` was a misread of the API — the actual switch is `extra_body.chat_template_kwargs.enable_thinking`, false. Without that, the model wastes its token budget on `<think>...</think>` tokens and either returns content with the answer wrapped in think-blocks or routes the entire output to a separate `message.reasoning_content` field, leaving `message.content` empty. We saw the empty-completion case on the first /translate test. The handler now (1) sends the correct enable_thinking=false flag, (2) reads `message.content` and strips any leftover `<think>...</think>` blocks, and (3) falls back to `message.reasoning_content` if content is empty. Empty-response error message now also includes the first 500 chars of the actual response shape so future debugging doesn't require a code change to inspect what NIM returned.

### Fixed (NIM model identifier — was 404)
- `annotator/google_apps_script.gs`, `docs/decisions/0007-translation-backend.md`, `annotator/SETUP.md`: `TRANSLATION_MODEL` default corrected from the non-existent `nvidia/nemotron-3-nano-omni` to the actual NIM model `nvidia/nemotron-3-nano-30b-a3b` (text-only Nano variant). The multimodal Omni model on NIM is `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning` and is reserved for the Phase 5 voice modality work; for Phase 1 text translation the smaller Nano variant is faster and cheaper at equivalent text quality. The original short identifier returned NIM 404 on every Suggest-translation call.

### Added (post-deploy access probe — catches the sticky Apps Script reversion)
- `.github/workflows/deploy-backend.yml`: new "Probe live deployment access" step runs after `clasp redeploy`. Curls the live `/exec?text=__probe__` URL (from a new `WEB_APP_URL` GitHub secret) and fails the build if the response isn't a JSON `{}` — i.e. if Apps Script silently reverted the deployment's "Who has access" setting from "Anyone" to a setting that requires auth. Surfaces the failure with a clear remediation instruction (open Manage Deployments → flip access → re-run) instead of letting the broken deployment propagate to the annotator. Catches the third instance of this exact bug seen on May 2-3.
- `DEPLOY.md`: Step 5 expanded to include the new `WEB_APP_URL` secret with rationale.
