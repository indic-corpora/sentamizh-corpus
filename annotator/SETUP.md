...
3. Add these properties:
   ...
   - **`TRANSLATION_MODEL`** *(optional, defaults to `meta/llama-3.3-70b-instruct`)*: any model identifier supported by NVIDIA NIM. Useful if you want to swap models without redeploying. The default is Llama 3.3 70B Instruct — non-reasoning, broad multilingual coverage, produces clean direct answers. The Phase 5 strategic target (NVIDIA Nemotron 3 Nano Omni, identifier `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning`) is a reasoning model whose chain-of-thought leaks into the response on NIM, so it's not a useful Phase 1 default. The eval in [`docs/decisions/0008-open-model-eval-classical-tamil.md`](../docs/decisions/0008-open-model-eval-classical-tamil.md) compares candidates and will inform the eventual production winner. The reasoning for the default and the eval methodology for picking a different one are recorded in [`docs/decisions/0007-translation-backend.md`](../docs/decisions/0007-translation-backend.md) and [`0008-open-model-eval-classical-tamil.md`](../docs/decisions/0008-open-model-eval-classical-tamil.md).
